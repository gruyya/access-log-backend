...
const UpdateGate = ({ barracks, gate }) => {
  const { data, setData, post, processing, errors, reset } = useForm({
    name: gate.name,
    barrack_id: gate.barrack.id,
  ...
  const submit = e => {
    ...
    post(route('gates.update', gate.id), {
      onFinish: () => route('gates.list'),
    });
  };
    ...
    <AuthenticatedLayout
      header={
        <h2 className="text-xl font-semibold leading-tight text-gray-800">
          Izmeni podatke o ulazu/prijavnici
        </h2>
      }
    >
                  ...
                  >
                    {barracks.map(barrack => (
                      <option
                        key={barrack.id}
                        value={barrack.id}
                        selected={barrack.id === data.barrack_id}
                      >
                        {barrack.name}
                      </option>
                    ))}
